...
public class GroupRentalController{
 ...
 @PostMapping @ResponseStatus(HttpStatus.CREATED) @Transactional public GroupRentalRequest create(@RequestHeader("X-Actor-Role")String role,@RequestHeader("X-User-Id")Long userId,@Valid @RequestBody CreateRequest r){roles.require(role,ActorRole.ROLE_CUSTOMER);if(r.endDate().isBefore(r.startDate()))throw new ResponseStatusException(HttpStatus.BAD_REQUEST,"이용 종료일은 시작일과 같거나 이후여야 합니다.");references.ensureUser(userId,ActorRole.ROLE_CUSTOMER);String summary=r.contactName()+"\n"+r.contactPhone()+"\n"+r.requestedItems();RentalOrder order=orders.save(new RentalOrder(userId,r.startDate(),r.endDate(),summary));return requests.save(new GroupRentalRequest(order,r.purpose(),r.headcount()));}
 ...
 public record CreateRequest(@NotBlank String purpose,@NotNull LocalDate startDate,@NotNull LocalDate endDate,@Min(2)int headcount,@NotBlank String requestedItems,@NotBlank String contactName,@NotBlank String contactPhone){}
}
