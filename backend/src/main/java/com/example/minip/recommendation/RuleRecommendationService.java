...
public class RuleRecommendationService {
    ...
    public RuleRecommendationService(ProductRepository products) {
    ...
    public RecommendationResponse recommend(RecommendationRequest request) {
        ...
        int rentalDays = (int) ChronoUnit.DAYS.between(
            request.rentalStartDate(), request.rentalEndDate()) + 1;
    ...
    private void validate(RecommendationRequest request) {
        ...
        if (request.purpose() == RentalPurpose.EVENT) {
            int groupCount = request.groupSizes() == null ? 0
                : request.groupSizes().values().stream()
                    .mapToInt(value -> value == null ? 0 : value).sum();
            if (groupCount < 2) {
                badRequest("단체 대여는 사이즈별 수량을 합해 2명 이상이어야 합니다.");
            }
        }
    }

    private RecommendationResponse.ProductResult toResult(Product product) {
        return new RecommendationResponse.ProductResult(product.getId(), product.getCategory(),
            product.getName(), product.getDescription(), product.getRentalPrice().intValue(),
            product.getPurchasePrice(), product.getRecommendationReason(), product.getStock());
    }
